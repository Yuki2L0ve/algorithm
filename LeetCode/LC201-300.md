...
class Solution {
    public int minSubArrayLen(int target, int[] nums) {
        int n = nums.length, ans = Integer.MAX_VALUE, s = 0;
...
```java
class Solution {
    public boolean check(int[] nums, int l, int mid) {
        int cnt = 0;
        for (int x : nums)
            if (x >= l && x <= mid)
                ++ cnt;
