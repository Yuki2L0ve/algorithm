...
class Solution {
    ...
    public String intToRoman(int num) {
        RomanNumeral[] romanNumerals = {
            new RomanNumeral(1000, "M"), new RomanNumeral(900, "CM"), new RomanNumeral(500, "D"), new RomanNumeral(400, "CD"),
            new RomanNumeral(100, "C"), new RomanNumeral(90, "XC"), new RomanNumeral(50, "L"), new RomanNumeral(40, "XL"),
            new RomanNumeral(10, "X"), new RomanNumeral(9, "IX"), new RomanNumeral(5, "V"), new RomanNumeral(4, "IV"), new RomanNumeral(1, "I")
        };

        StringBuilder ans = new StringBuilder();
        for (RomanNumeral R : romanNumerals) {
            while (num >= R.value) {
                num -= R.value;
                ans.append(R.symbol);
            }
...
class Solution {
    public String longestCommonPrefix(String[] strs) {
        ...
        for (int i = 0; i < base.length(); ++ i) {
            for (String s : strs)
                if (i >= s.length() || s.charAt(i) != base.charAt(i))
                    return prefix;
            prefix += base.charAt(i);
        }
        return prefix;
    }
}
...
func dfs(start, target int) {
    ...
    for i := start; i < len(c) && target - c[i] >= 0; i ++ {
...
class Solution {
    ...
    void dfs(int start, int target) {
        ...
        for (int i = start; i < n && target - c[i] >= 0; ++ i) {
            path.add(c[i]);
            dfs(i, target - c[i], target);
    ...
    public int minPathSum(int[][] w) {
        ...
        for (int i = 1; i <= n; ++ i) {
            for (int j = 1; j <= m; ++ j) {
                ...
                f[i][j] = Math.min(f[i - 1][j], f[i][j - 1]) + w[i - 1][j - 1];
            }
        }
...
class Solution {
    ...
    public void sortColors(int[] nums) {
        int p0 = 0, i = 0, p2 = nums.length - 1, x = 1;
        while (i <= p2) {
            if (nums[i] < x) {
                swap(nums, i, p0);
                ++ p0;
