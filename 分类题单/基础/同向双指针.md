...
# LC2743. 计算没有重复字符的子字符串数量
[传送门](https://leetcode.cn/problems/count-substrings-without-repeating-character/description/)
```C++
class Solution {
public:
    int numberOfSpecialSubstrings(string s) {
        int n = s.size(), ans = 0, k = 1;
        unordered_map<char, int> mp;
        for (int l = 0, r = 0; r < n; ++ r) {
            ++ mp[s[r]];
            while (mp[s[r]] > k) {
